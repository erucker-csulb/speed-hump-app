...
namespace EquationButtons
{
    internal class EquationPalette_buttonAccHist : Button
    {
            ...
            QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    var myCommand = string.Format(@"/c """"{0}"" ""{1}""""",
                        ...
                        System.IO.Path.Combine(pathPython, @"D:\CSULB\SpeedHumps\speed-hump-app\accident_history_eq.py"));
    ...
    internal class EquationPalette_buttonPreScore : Button
    {
        protected override void OnClick()
        {
            QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    var myCommand = string.Format(@"/c """"{0}"" ""{1}""""",
                        System.IO.Path.Combine(pathProExe, "python.exe"),
                        System.IO.Path.Combine(pathPython, @"D:\CSULB\SpeedHumps\speed-hump-app\pre_score_eq.py"));
                    // Create process start info, with instruction settings
                    var procStartInfo = new System.Diagnostics.ProcessStartInfo("cmd", myCommand);
                    procStartInfo.RedirectStandardOutput = true;
                    procStartInfo.RedirectStandardError = true;
    ...
    internal class EquationPalette_buttonCalcSpeed : Button
    {
        protected override void OnClick()
        {
            QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    var myCommand = string.Format(@"/c """"{0}"" ""{1}""""",
                        System.IO.Path.Combine(pathProExe, "python.exe"),
                        System.IO.Path.Combine(pathPython, @"D:\CSULB\SpeedHumps\speed-hump-app\calc_speed_eq.py"));
                    // Create process start info, with instruction settings
                    var procStartInfo = new System.Diagnostics.ProcessStartInfo("cmd", myCommand);
                    procStartInfo.RedirectStandardOutput = true;
                    procStartInfo.RedirectStandardError = true;
    ...
    internal class EquationPalette_buttonCalcADT : Button
    {
        protected override void OnClick()
        {
            QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    var myCommand = string.Format(@"/c """"{0}"" ""{1}""""",
                        System.IO.Path.Combine(pathProExe, "python.exe"),
                        System.IO.Path.Combine(pathPython, @"D:\CSULB\SpeedHumps\speed-hump-app\calc_adt_eq.py"));
                    // Create process start info, with instruction settings
                    var procStartInfo = new System.Diagnostics.ProcessStartInfo("cmd", myCommand);
                    procStartInfo.RedirectStandardOutput = true;
                    procStartInfo.RedirectStandardError = true;
    ...
    internal class EquationPalette_buttonRankScore : Button
    {
        protected override void OnClick()
        {
            QueuedTask.Run(() =>
            {
                try
                {
                    ...
                    var myCommand = string.Format(@"/c """"{0}"" ""{1}""""",
                        System.IO.Path.Combine(pathProExe, "python.exe"),
                        System.IO.Path.Combine(pathPython, @"D:\CSULB\SpeedHumps\speed-hump-app\rank_score_eq.py"));
                    // Create process start info, with instruction settings
                    var procStartInfo = new System.Diagnostics.ProcessStartInfo("cmd", myCommand);
                    procStartInfo.RedirectStandardOutput = true;
                    procStartInfo.RedirectStandardError = true;
